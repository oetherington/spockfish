...
class Position {
	...
	// This data uniquely defines the position
	private turn: Color;
	private ply: number;
	...
	private enPassantSquare: FlatSquare | null;

	// This data is automatically generated by the constructor
	...
	constructor(
		...
		enPassantSquare: FlatSquare | null = null,
	...
	private generateLegalMovesForPiece(p: Piece) : PieceMove[] {
		...
		let enPassant = new FlatBitboard();
		if (piece === 'p' &&
				this.enPassantSquare &&
				this.enPassantSquare.rank === rank &&
				fileDiff(this.enPassantSquare.file, file) === 1)
			enPassant.setSquare(
				this.enPassantSquare.file,
				this.enPassantSquare.rank + (this.turn === 'w' ? 1 : -1),
			);
		...
		for (const targetLevel of this.levels) {
			...
			let passant = enPassant.both(boardSquares[targetLevel]);
			...
			const passantMoves = passant.toMoves(
				'p', color, from, targetLevel, true, this.levels, true);

			result = result
				.concat(captureMoves).concat(quietMoves).concat(passantMoves);
		}
